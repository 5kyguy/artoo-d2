...
The **R2-D2 menu** is implemented by `bin/r2-d2-menu`. It is launched with **Super + Alt + Space** or by clicking the R2-D2 icon in Waybar. You can open a specific submenu and exit after one action: `r2-d2-menu <submenu>`, e.g. `r2-d2-menu install`, `r2-d2-menu system`, `r2-d2-menu share`, `r2-d2-menu screenrecord`, `r2-d2-menu power`. The Waybar battery icon opens the Power profile submenu (`r2-d2-menu power`).

---

## Main menu

The main menu offers: **Trigger**, **Setup**, **Install**, **Remove**, **Update**, **About**, **System**.

| Entry | Action |
| ----- | ------ |
| **Trigger** | Screenshot, Screenrecord, Share (clipboard/file/folder), Toggle (screensaver, nightlight, idle lock, top bar, device display) |
| **Setup** | Audio, Wifi, Bluetooth, Power profile, System sleep (enable/disable suspend/hibernate), Monitors, Keybindings (edit `~/.config/hypr/bindings.conf`), Input (edit `~/.config/hypr/input.conf`), DNS, Security (Fingerprint, Fido2) |
| **Install** | See [Install submenu](#install-submenu) |
| **Remove** | See [Remove submenu](#remove-submenu) |
| **Update** | R2-D2, Firmware, Password (drive encryption / user), Timezone, Time |
...
| **Editor** | VSCode, Zed. Cursor is preinstalled when AUR base packages are used. |
| **Terminal** | Alacritty, Ghostty (set as default terminal for R2-D2). |
...
To change the wallpaper, use the background selector: **Super + Ctrl + Space** (Walker menu).
...
| **Web App** | Remove web apps (`r2-d2-webapp-remove`). |
| **TUI** | Remove TUI shortcuts (`r2-d2-tui-remove`). |
